...
const product_index = async (req, res) => {
    ...
    let categories = (await db.collection("settings").findOne({ name: "general" })).categories
...
const product_create_post = (req, res) => {
    upload.array('images')(req, res, async (err) => {
        ...
        for (let i = 0; i < req.files.length; i++) {
            try {
                const thumbnail = await imageThumbnail(req.files[i].buffer, { withMetaData: true });
        ...
        db.collection("settings").findOneAndUpdate({ name: "general" }, {
